...
// address for values not stored as variables
...
func FindPropAddr(obj *ObjectVal, prop RuntimeVal) PropAddr {
...
func GetObjectProp(val RuntimeVal, prop RuntimeVal) RuntimeVal {
...
func SetObjectProp(val RuntimeVal, prop RuntimeVal, value RuntimeVal) {
	var obj *ObjectVal
	switch v := val.(type) {
	case *ObjectVal:
		obj = v
...
func setOwnPropertyDescriptor(addr PropAddr, obj *ObjectVal, key RuntimeVal, value RuntimeVal, pd PropertyDescriptor) {
	if addr == -1 {
		addr = AllocAddr()
	}
	pd.addr = addr
...
type (
	...
	Promise     struct{}
	ObjectProps struct {
		members *Map[RuntimeVal, PropertyDescriptor]
	}
...
func (s StringVal) Inspect(depth int, _ *Interpreter, _ *Scope) string {
	...
	return "\x1b[32m" + escapeString(s.Value()) + "\x1b[0m"
}
...
func NewObjectProps() *ObjectProps {
	return &ObjectProps{
		members: NewMap[RuntimeVal, PropertyDescriptor](),
	}
}
...
func init() {
	...
	iterator := MK_SYMBOL(NativeIteratorSymbol)
	symbol_table.set("iterator", iterator)

	toPrimitive := MK_SYMBOL("toPrimitive")
	symbol_table.set("toPrimitive", toPrimitive)
}
...
func (obj *ObjectVal) Inspect(depth int, r *Interpreter, s *Scope) string {
	...
	if sym, exists := symbol_table.get("debug"); exists {
		v, ok := r.callMethod(obj, sym, RuntimeArgs{StringVal(sep)}, s).(StringVal)
		if ok {
			return string(v)
		}
	}
