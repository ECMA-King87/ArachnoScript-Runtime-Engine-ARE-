...
func (r *Interpreter) Interpret() {
	...
	r.DeclareVar("module", "constant", &ScopeObject{program_scope}, Loc{
		start: 0,
		end:   0,
		col:   1,
		line:  1,
...
func (r *Interpreter) Eval_trad_for_loop(stmt TradForLoop, body []NodeIndex, s *Scope) {
	...
	io.Println(io.Sprintf("DEBUG: Eval_trad_for_loop before.tag=%s children=%d", before.tag, len(before.children)))
	r.Evaluate(before, loop_scope)
	// Debug: list declared variables in loop scope
	loop_scope.vars.forEach(func(k string, v Addr) {
		io.Println(io.Sprintf("DEBUG: loop scope var -> %s = %d", k, v))
	})
	// cont:
	for r.toBool(r.Evaluate(condition, loop_scope)) {
		if len(body) > 0 {
...
func (r *Interpreter) EvalWhileStmt(node Node, s *Scope) RuntimeVal {
	...
	loop:
		...
		if r.toBool(r.Evaluate(r.getNode(stmt.condition), s)) {
			goto loop
		}
	} else {
		for r.toBool(r.Evaluate(r.getNode(stmt.condition), s)) {
			block_scope := newEnv(s.path, "loop", s, s.enclosing_object)
			if len(node.children) > 0 {
				r.EvalBlock(node.children, block_scope)
			}
...
func (r *Interpreter) EvalFunDecl(node Node, s *Scope) RuntimeVal {
	...
	declEnv := s
	body := r.getNode(decl.body).children
	fn := MK_FUNCTION(decl.name, body, decl.params, declEnv, node.loc, decl.async, decl.anonymous, decl.arrow, r.parser)
	if fn.anonymous {
		return fn
	}
...
func (r *Interpreter) getObjectMemberRef(object RuntimeVal, member StringVal, loc Loc, s *Scope) PropAddr {
	...
	case *NativeClass:
		...
		return addr
	case *ArrayVal:
		if i, err := strconv.ParseInt(string(member), 10, 64); err == nil {
			index := int(i)
			if index < 0 {
				index += v.len
			}
...
func (r *Interpreter) GetMember(value RuntimeVal, member StringVal) RuntimeVal {
...
func (r *Interpreter) Eval_ternary_expr(node Node, s *Scope) RuntimeVal {
	...
	cond := r.toBool(r.Evaluate(r.getNode(expr.condition), s))
	if cond {
			...
			if member.value != -1 && member_node.tag == "Function Decl" {
				...
				if FindPropAddr(proto, key) == -1 {
					...
					if pd._type_.accessor == false {
						...
						if !exists {
							r.ThrowSourceError("InternalError", "property descriptor address is invalid", method.loc, method.declEnv)
...
func (r *Interpreter) GetInstanceProto(inst *Instance, p RuntimeVal, member StringVal, loc Loc, env *Scope) RuntimeVal {
...
func (r *Interpreter) GetInstanceProtoWithMember(inst *Instance, p RuntimeVal, member StringVal, loc Loc, env *Scope, bypass bool) *ObjectVal {
	proto, ok := p.(*ObjectVal)
	if ok {
		if des, e := proto.props.members.get(toValidPropKey(member)); e {
			// no errors (only the type checker will show error)
...
func (r *Interpreter) SetInstanceProto(inst *Instance, p RuntimeVal, key StringVal, value RuntimeVal, loc Loc, env *Scope) bool {
	proto, ok := p.(*ObjectVal)
	if !ok {
...
func (r *Interpreter) Eval_call_expr(node Node, s *Scope) RuntimeVal {
	...
	return r.CallFunction(caller, args, s, caller_node.loc, false, r.extractThis(caller_node, s))
}
...
func (r *Interpreter) toBool(cond_value RuntimeVal) bool {
...
func (r *Interpreter) ThrowSourceError(name, msg string, loc Loc, env *Scope) {
	r.ThrowWithName(name, io.Sprintf("%s%s", msg, SourceLog(env.path, loc)), env)
}
...
func (r *Interpreter) resolve(varname string, s *Scope, loc Loc, b *Scope) *Scope {
	...
	if s.parent == nil {
		// Debug: failed to resolve variable - dump current scope chain information
		io.Println(io.Sprintf("DEBUG: resolve failed for '%s' in scope (type=%s path=%s)", varname, s._type_, s.path))
		s.vars.forEach(func(k string, v Addr) {
			io.Println(io.Sprintf("DEBUG: scope var -> %s = %d", k, v))
		})
		if s.parent != nil {
			io.Println(io.Sprintf("DEBUG: parent scope type=%s path=%s", s.parent._type_, s.parent.path))
		}
...
func (r *Interpreter) lookup(varname string, s *Scope, loc Loc) RuntimeVal {
	...
	addr, _ := scope.vars.get(varname)
	if addr == 0 {
		r.ThrowSourceError("InternalError", "variable "+varname+" declared but has no address", loc, s)
	}
